...
event1_ggH = {
  "mothers":Mela.SimpleParticleCollection_t([21, 21], [0, 0], [0, 0], [5.37086667033E+02, -7.27293062837E+00], [5.37086667033E+02, 7.27293062837E+00], False),
  
  "daughters":Mela.SimpleParticleCollection_t(
    [11, -11, 12, -12], 
    [-4.96385765951E+00, 3.89098702611E+00, 8.04215953543E+00, -6.96928890202E+00], 
    [ 1.62933935971E+01, 4.04799485238E+00,-2.29213235677E+01, 2.57993511828E+00],
    [2.11696003997E+02, 1.06735429842E+00, 2.77968427622E+01, 2.89253535347E+02],
    [2.12380113632E+02, 5.71535544142E+00, 3.69151442044E+01, 2.89348984384E+02]
  ),
...
event2_VBF = {
  "mothers":Mela.SimpleParticleCollection_t([-4, 1], [0, 0], [0, 0], [ 3.03951895455E+01, -7.16454865818E+02], [ 3.03951895455E+01, 7.16454865818E+02]),
  "daughters":Mela.SimpleParticleCollection_t(
    [-11, 11, -13, 13],
    [ 3.31752759767E+00, -3.75597136210E+00,  6.93884009242E+01, -1.52862975212E+01],
    [ 1.23639112123E+00, -1.98687314816E+01,  3.00845351746E+01,  9.17645398978E+00],
    [-8.43121325514E+00, -8.72265512647E+01, -2.00352143436E+02, -2.36184204771E+02],
    [9.14439771558E+00, 8.95396284186E+01, 2.14151399675E+02, 2.36856217877E+02]
  ),
  "associated":Mela.SimpleParticleCollection_t(
    [-4, 1], [-6.29623305954E+01, 9.29867095677E+00], [-4.67693451618E+01, 2.61406963578E+01], [-5.85204921720E+01, -9.53450713739E+01], [9.78584422772E+01, 9.92999694004E+01]
  )
}
...
event3_ZH = {
  "mothers": Mela.SimpleParticleCollection_t([1, -1], [0, 0], [0, 0], [ 5.98124082110E+01, -3.44771438947E+02], [ 5.98124082110E+01, 3.44771438947E+02]),
  "daughters":Mela.SimpleParticleCollection_t(
    [-11, 11, -11, 11],
    [ 1.78742924427E+01, 3.86248305415E+01, 5.37669110106E+00, -4.84989558626E+01],
    [-1.12102312034E+01, -3.12040320381E+01, -4.15860490296E-01, -4.60240644895E+01],
    [-2.03424438331E+00, -4.96421050079E+01, -7.57669704638E+00, -1.04352390245E+02],
    [2.11966451221E+01, 7.02133017289E+01, 9.29989707133E+00, 1.23934598003E+02]
  ),
  "associated":Mela.SimpleParticleCollection_t(
    [13, -13], [ 3.17738029751E+01, -4.51506611978E+01], [-1.54337455361E+00, 9.03975627749E+01], [-2.87056115717E+01, -9.26479824805E+01], [4.28483355599E+01, 1.37091069673E+02]
  )
}
...
class TestMela(unittest.TestCase):
  @classmethod
  def setUpClass(cls):
    cls.m = Mela.Mela(13, 125)
  def setUp(self):
    random.seed(seed)
  def tearDown(self):
    self.m.resetInputEvent()
  def testcontact_decay(self):
    self.runcontact(event1_ggH, False, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.ZZINDEPENDENT)
  def testcontact_VBF(self):
    self.runcontact(event2_VBF, True, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.JJVBF)
  def testcontact_ZH(self):
    self.runcontact(event3_ZH, True, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.Lep_ZH)
  def runcontact(self, event, isprod, *setprocessargs):
    ...
    m.setInputEvent(event["daughters"], event["associated"], event["mothers"], True)

    m.setProcess(*setprocessargs)
    m.ghz1 = [ghz1, 0]
    m.ghz1_prime2 = [ghz1_prime2, 0]
    m.ghzgs1_prime2 = [ghzgs1_prime2, 0]
    me1 = computeP(True)

    m.setProcess(*setprocessargs)
    m.ghz1 = (ghz1 + 2 * ghz1_prime2 * (M_Z**2 - 1j*M_Z*Ga_Z)/L1**2).real, (ghz1 + 2 * ghz1_prime2 * (M_Z**2 - 1j*M_Z*Ga_Z)/L1**2).imag
    m.ghzzp1 = [M_Z**2/L1**2, 0]
    m.ezp_El_left = m.ezp_Mu_left = m.ezp_Ta_left = [aL_lep * ghz1_prime2 + e * ghzgs1_prime2, 0]
    m.ezp_El_right = m.ezp_Mu_right = m.ezp_Ta_right = [aR_lep * ghz1_prime2 + e * ghzgs1_prime2, 0]
    m.ezp_Up_left = m.ezp_Chm_left = m.ezp_Top_left = [aL_up * ghz1_prime2 + c_up * ghzgs1_prime2, 0]
    m.ezp_Up_right = m.ezp_Chm_right = m.ezp_Top_right = [aR_up * ghz1_prime2 + c_up * ghzgs1_prime2, 0]
    m.ezp_Dn_left = m.ezp_Str_left = m.ezp_Bot_left = [aL_dn * ghz1_prime2 + c_dn * ghzgs1_prime2, 0]
    m.ezp_Dn_right = m.ezp_Str_right = m.ezp_Bot_right = [aR_dn * ghz1_prime2 + c_dn * ghzgs1_prime2, 0]
    m.ezp_NuE_left = [aL_nu * ghz1_prime2, 0]
    m.ezp_NuE_right = [aR_nu * ghz1_prime2, 0]
    me2 = computeP(True)

    self.assertEqual(me1, me2)
    self.assertNotEqual(me1, 0)
  ...
  def testzp_decay(self):
    self.runVprime(event1_ggH, False, False, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.ZZINDEPENDENT, **self.Vprimekwargs(0, False))
  def testzp_spin2(self):
    self.runVprime(event1_ggH, False, False, Mela.Process.SelfDefine_spin2, Mela.MatrixElement.JHUGen, Mela.Production.ZZINDEPENDENT, **self.Vprimekwargs(2, False))
  def testzp_VBF(self):
    self.runVprime(event2_VBF, True, False, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.JJVBF, **self.Vprimekwargs(60, False))
  def testzp_ZH(self):
    self.runVprime(event3_ZH, True, False, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.Lep_ZH, **self.Vprimekwargs(50, False))
  def testzpzp_decay(self):
    self.runVprime(event1_ggH, False, True, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.ZZINDEPENDENT, **self.Vprimekwargs(0, True))
  def testzp_spin2(self):
    self.runVprime(event1_ggH, False, True, Mela.Process.SelfDefine_spin2, Mela.MatrixElement.JHUGen, Mela.Production.ZZINDEPENDENT, **self.Vprimekwargs(2, True))
  def testzpzp_VBF(self):
    self.runVprime(event2_VBF, True, True, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.JJVBF, **self.Vprimekwargs(60, True))
  def testzpzp_ZH(self):
    self.runVprime(event3_ZH, True, True, Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.Lep_ZH, **self.Vprimekwargs(50, True))
  def runVprime(self, event, isprod, usevpvp, *setprocessargs, **couplings):
    ...
    for coupling, value in couplings.items():
      ...
      setattr(m, coupling, [value.real, value.imag])
    ...
    for coupling, value in couplings.items():
      ...
      setattr(m, newcoupling, [value.real, value.imag])
    m.M_Zprime = M_Z
    m.Ga_Zprime = Ga_Z
    m.M_Wprime = M_W
    m.Ga_Wprime = Ga_W

    m.ezp_El_left = m.ezp_Mu_left = m.ezp_Ta_left = [aL_lep, 0]
    m.ezp_El_right = m.ezp_Mu_right = m.ezp_Ta_right = [aR_lep, 0]
    m.ezp_Up_left = m.ezp_Chm_left = m.ezp_Top_left = [aL_up, 0]
    m.ezp_Up_right = m.ezp_Chm_right = m.ezp_Top_right = [aR_up, 0]
    m.ezp_Dn_left = m.ezp_Str_left = m.ezp_Bot_left = [aL_dn, 0]
    m.ezp_Dn_right = m.ezp_Str_right = m.ezp_Bot_right = [aR_dn, 0]
    m.ezp_NuE_left = [aL_nu, 0]
    m.ezp_NuE_right = [aR_nu, 0]

    m.ewp_El_left = m.ewp_Mu_left = m.ewp_Ta_left = m.ewp_Up_left =  m.ewp_Chm_left = m.ewp_Top_left = [bL, 0]
  ...
  def testmultiplecalls_ZH(self):
    temp = {}
    for i,j in event3_ZH.items():
      temp[i] = j
    temp["associated"] = Mela.SimpleParticleCollection_t(
    [3, -3], [ 3.17738029751E+01, -4.51506611978E+01], [-1.54337455361E+00, 9.03975627749E+01], [-2.87056115717E+01, -9.26479824805E+01], [4.28483355599E+01, 1.37091069673E+02]
  )
    self.runmultiplecalls(temp)

  def runmultiplecalls(self, event):
    m = self.m
    m.setInputEvent(event["daughters"], event["associated"], event["mothers"], False)

    firstrun = []
    secondrun = []

    for lst in firstrun, secondrun:
      m.setProcess(Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.JJVBF)
      m.ghz1 = [1, 0]
      lst.append(m.computeProdP(True))
      lst.append(m.getConstant())

      m.setProcess(Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.JJQCD)
      m.ghg2 = [1, 0]
      lst.append(m.computeProdP(True))
      lst.append(m.getConstant())
      ...
      m.setProcess(Mela.Process.SelfDefine_spin0, Mela.MatrixElement.JHUGen, Mela.Production.ZZINDEPENDENT)
      m.ghzzp1 = ezp_El_left = [1, 0]
