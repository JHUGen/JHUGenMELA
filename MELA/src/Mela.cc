...
/**
 * @file Mela.cc
 * @brief The driving code for the MELA interface to MCFM/JHUGen-MELA
 * @warning Each specific type of computeP* function comes with its wrapper for common use. 
 * @warning Removing these wrappers from Mela will not introduce any bugs, but it might affect packages that depend on it (eg. MEMCalculators).
 * @warning Please adhere to the following coding conventions
 * @warning Never put return statements in the middle of the computeP* functions unless it is a wrapper. 
 * @warning Functions calling the Mela::ZZME member have to reset the couplings, so an abrupt termination does not reset the couplings properly.
*/
